...
async function run() {
  ...
  const tmpDir = path.join(ARTIFACT_DIR, 'scratch', 'chrome-offers-full');
  ...
  try {
    ...
    await cdp.captureScreenshot('enhanced_coupons_centered.png');

    // Scroll slightly more to capture the second row
    await cdp.evaluate(`
      window.scrollBy(0, 450);
    `);
    await sleep(600);
    await cdp.captureScreenshot('enhanced_coupons_row2.png');
